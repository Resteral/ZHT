...
DECLARE
    ...
    v_avg_elo_rating_team1 NUMERIC;
    v_avg_elo_rating_team2 NUMERIC;
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_consensus_count FROM tournament_participants WHERE tournament_id = p_tournament_id
    ...
    IF v_participant_count > 0 AND v_participant_count = v_consensus_count THEN
        -- Get tournament data
        SELECT prize_pool INTO v_tournament_prize_pool FROM tournaments WHERE id = p_tournament_id;

        -- Identify teams by draft_order
        SELECT id INTO v_team1_id FROM tournament_teams WHERE tournament_id = p_tournament_id AND (draft_order = 1 OR team_name ILIKE '%Team 1%') LIMIT 1;
        SELECT id INTO v_team2_id FROM tournament_teams WHERE tournament_id = p_tournament_id AND (draft_order = 2 OR team_name ILIKE '%Team 2%') LIMIT 1;
        ...
        SELECT COALESCE(AVG(u.elo_rating), 1200) INTO v_avg_elo_rating_team1 
        FROM public.users u 
        JOIN tournament_team_members tm ON u.id = tm.user_id 
        WHERE tm.team_id = v_team1_id;
        
        SELECT COALESCE(AVG(u.elo_rating), 1200) INTO v_avg_elo_rating_team2 
        FROM public.users u 
        JOIN tournament_team_members tm ON u.id = tm.user_id 
        WHERE tm.team_id = v_team2_id;

        -- Expected scores
        v_exp_score1 := 1 / (1 + POWER(10, (v_avg_elo_rating_team2 - v_avg_elo_rating_team1) / 400.0));
        ...
        IF v_team1_id IS NOT NULL THEN
            SELECT COUNT(*) INTO v_winner_count FROM tournament_team_members WHERE team_id = v_team1_id;
            IF v_winner_count > 0 THEN
                ...
                FOR v_record IN SELECT user_id FROM tournament_team_members WHERE team_id = v_team1_id LOOP
                    ...
                    SELECT 
                        v_record.user_id, 
                        (SELECT user_id FROM tournament_team_members WHERE team_id = v_team2_id LIMIT 1), 
                        ...
                        v_elo_change1, NOW()
                    FROM users u WHERE u.id = v_record.user_id;
                    ...
                    IF v_payout_per_player > 0 THEN
                        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'financial_transactions') THEN
                            INSERT INTO financial_transactions (user_id, amount, transaction_type, status, description, processed_at)
                        ...
                        ELSEIF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transactions') THEN
                            INSERT INTO transactions (user_id, amount, type, provider, status, external_id, created_at)
        ...
        IF v_team2_id IS NOT NULL THEN
            SELECT COUNT(*) INTO v_winner_count FROM tournament_team_members WHERE team_id = v_team2_id;
            IF v_winner_count > 0 THEN
                v_payout_per_player := CASE 
                ...
                FOR v_record IN SELECT user_id FROM tournament_team_members WHERE team_id = v_team2_id LOOP
                    ...
                    SELECT 
                        v_record.user_id, 
                        (SELECT user_id FROM tournament_team_members WHERE team_id = v_team1_id LIMIT 1), 
                        'Zealot Hockey', 'tournament', p_tournament_id,
