...
import TournamentAuthGate from "@/components/auth/tournament-auth-gate"
...
export default function CreateTournamentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!currentUser) {
        throw new Error("Authentication required to create tournaments. Please sign in and try again.")
      }

      console.log("[v0] Authenticated user:", currentUser.id)

      let { data: dbUser } = await supabase.from("users").select("id, username").eq("id", currentUser.id).single()
      ...
      const tournament = await monthLongTournamentService.createMonthLongTournament(tournamentData, currentUser.id)
  ...
  return (
    <TournamentAuthGate requireAuth={true}>
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Maximum number of players that can join this tournament
                  </p>
                </div>
              ...
              <Card
                className={`${hasConflict || !isDateValid ? "bg-destructive/10 border-destructive" : "bg-muted/50"}`}
              >
                <CardContent className="pt-6">
                  <div className="space-y-2">
                    ...
                    <div className="text-sm space-y-1">
                      ...
                      <p>
                        <strong>{formData.settings.draft_mode.replace("_", " ").toUpperCase()}</strong> tournament
                        format
                      </p>
                      <p>
