...
export default function Draft5v5Page() {
  ...
  const loadLobbies = async () => {
    try {
      console.log("[v0] Loading 5v5 lobbies...")
      ...
      console.log("[v0] Loaded 5v5 matches:", matches)
      setLobbies(matches || [])
    } catch (err) {
  ...
  const createLobby = async () => {
    setCreating(true)
    try {
      console.log("[v0] Creating 5v5 lobby...")
      ...
      console.log("[v0] Created 5v5 lobby:", match.id)
  ...
  if (loading) {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">Loading 5v5 lobbies...</div>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 mb-8">
        <Card className="border-2 border-dashed border-primary/20 hover:border-primary/40 transition-colors">
          <CardHeader className="text-center">
            ...
            <CardDescription>Start a new 5v5 draft lobby</CardDescription>
          </CardHeader>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-semibold">Active 5v5 Lobbies</h2>

        {lobbies.length === 0 ? (
          <Card>
            <CardContent className="text-center py-8">
              <Users className="h-12 w-12 mx-auto mb-4 text-muted-foreground" />
              <p className="text-muted-foreground">No active 5v5 lobbies</p>
                ...
                <Card key={lobby.id} className="hover:shadow-md transition-shadow">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="secondary">{currentParticipants}/10</Badge>
                    </div>
                    <CardDescription>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      <div className="text-sm">
                        <div className="font-medium mb-2">Players ({currentParticipants}/10):</div>
                        <div className="space-y-1">
                          ...
                          {currentParticipants > 5 && (
                            <div className="text-xs text-muted-foreground">+{currentParticipants - 5} more...</div>
                          )}
                        </div>
                      </div>
                      ...
                      <Button
                        ...
                        className="w-full"
                        disabled={currentParticipants >= 10}
                        variant={isUserInLobby ? "outline" : "default"}
                      >
                        {currentParticipants >= 10 ? "Full" : isUserInLobby ? "Enter Lobby" : "Join Lobby"}
                      </Button>
                    </div>
                  </CardContent>
                </Card>
