...
export default function ThreeVThreePage() {
  ...
  const loadLobbies = async () => {
    try {
      console.log("[v0] Loading 3v3 lobbies...")
  ...
  const createLobby = async () => {
    setCreating(true)
    try {
      console.log("[v0] Creating 3v3 lobby...")
    ...
    <div className="container mx-auto py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <Card className="border-dashed border-2 hover:border-primary/50 transition-colors">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-muted-foreground text-center mb-4">Strategic 3v3 matches with captain draft</p>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm">{currentParticipants}/6 Players</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Clock className="h-4 w-4" />
                    <span className="text-sm">60s picks</span>
                  </div>
                </div>
                ...
                <Button
                  ...
                  className="w-full"
                  disabled={currentParticipants >= 6}
                  variant={isUserInLobby ? "outline" : "default"}
                >
                  {currentParticipants >= 6 ? "Full" : isUserInLobby ? "Enter Lobby" : "Join Lobby"}
                </Button>
              </CardContent>
            </Card>
          )
      ...
      {lobbies.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold mb-2">No Active 3v3 Matches</h3>
          <p className="text-muted-foreground mb-4">Be the first to create a 3v3 lobby!</p>
        </div>
      )}

      {loading && (
        <div className="text-center py-12">
          <div className="text-muted-foreground">Loading 3v3 lobbies...</div>
        </div>
      )}
    </div>
  )
