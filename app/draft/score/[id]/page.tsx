...
export default function ScoreScreenPage({ params }: ScoreScreenPageProps) {
  ...
  useEffect(() => {
    const checkForConsensus = async () => {
      if (!submissions.length || !matchData || matchData.status === "completed") return
      ...
      if (largestGroup.length >= requiredConsensus) {
        console.log("[v0] Consensus reached automatically, completing match...")
        await completeMatch(largestGroup[0])
      }
    }

    checkForConsensus()
  }, [submissions, matchData])
